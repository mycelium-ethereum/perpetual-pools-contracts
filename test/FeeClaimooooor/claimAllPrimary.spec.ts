...
describe.only("FeeClaimooooor - claimAllPrimary", () => {
    ...
    context("Single pool", () => {
        ...
        it("claims", async () => {
            const balanceBefore = await token.balanceOf(feeAddress)
            const feeAmount = await pool.primaryFees()
            await feeClaimooooor.claimAllPrimary()
            const balanceAfter = await token.balanceOf(feeAddress)

            expect(balanceAfter).to.equal(balanceBefore.add(feeAmount))
        })
    })
    ...
    context("Multiple pools", () => {
        ...
        it("claims", async () => {
            ...
            const balanceBefore = await token.balanceOf(feeAddress)
            let feeAmount = (await pool1.primaryFees()).add(
