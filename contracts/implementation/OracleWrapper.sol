...
contract OracleWrapper is IOracleWrapper, AccessControl {
   ...
   */
    ...
    function setOracle(string memory marketCode, address oracle) external override onlyOperator {
        require(oracle != address(0), "Oracle cannot be 0 address");
        assetOracles[marketCode] = oracle;
    }

    function getPrice(string memory marketCode) external view override returns (int256) {
        (, int256 price, , uint256 timeStamp, ) = AggregatorV2V3Interface(assetOracles[marketCode]).latestRoundData();
